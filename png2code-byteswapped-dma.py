...
# RGB value 16bit(RGB:565)convert to text
def rgb2hexstr(rgb):
    col = ((rgb[0]>>3)<<11) | ((rgb[1]>>2)<<5) | (rgb[2]>>3)
    col_swapped = ((col >> 8) | (col << 8)) & 0x0000FFFF
    return "0x{:04X}".format(col_swapped)
#    return "0x{:04X}".format(col)

with open(in_name, 'rb') as infile:
  with open(out_name, 'wb+') as outfile:
    outfile.write('#include <stdint.h>\n\n\n'.encode('utf-8'))
    arrary_name = 'const uint16_t ' + out_name[0:out_name.find('.')] + '[] = {\n'
